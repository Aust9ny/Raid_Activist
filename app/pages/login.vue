  ...
  >
    ...
    <div class="relative z-10 w-full max-w-md animate-in">
      <transition name="fade">
        <div
          v-if="isSuccess"
          class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-indigo-950/80 backdrop-blur-2xl rounded-[3rem] border border-emerald-500/30"
        >
          <span class="text-6xl animate-bounce mb-4">🚀</span>
          <h2
            class="text-white font-black text-2xl uppercase italic tracking-tighter"
          >
            Identity Verified
          </h2>
          <p
            class="text-white/40 text-[10px] font-black uppercase tracking-[0.3em] mt-2"
          >
            Opening Data Stream...
          </p>
        </div>
      ...
      <div
        class="glass-panel p-10 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.4)] border-white/10 space-y-8"
        :class="{
          'opacity-20 scale-95 pointer-events-none transition-all duration-700':
            isSuccess,
        }"
      >
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          <div class="space-y-1">
            <label
              class="text-[9px] font-black text-pink-300 uppercase ml-4 tracking-widest"
              >Access Email</label
            >
            <input
              v-model="email"
          ...
          <div class="space-y-1">
            <div class="flex justify-between items-center px-4">
              <label
                class="text-[9px] font-black text-pink-300 uppercase tracking-widest"
                >Access Key</label
              >
              <button
                type="button"
                @click="showPassword = !showPassword"
                class="text-[9px] font-black text-white/20 hover:text-white transition-colors uppercase"
              >
