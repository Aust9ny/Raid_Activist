...
const isEmailValid = computed(
  () => email.value.length === 0 || emailPattern.test(email.value)
);
const isPhoneValid = computed(
  () => phoneNumber.value.length === 0 || phonePattern.test(phoneNumber.value)
);
const isNicknameValid = computed(
  () => nickname.value.length === 0 || nickname.value.trim().length > 2
);

const constraintChecks = computed(() => {
  return constraints.map((c) => ({
    ...c,
    meets: c.regex.test(password.value),
  }));
});

const isPasswordStrong = computed(() =>
  constraintChecks.value.every((c) => c.meets)
);
...
<template>
  <div
    class="auth-root min-h-screen flex items-center justify-center relative overflow-hidden p-4"
  >
    ...
    <div class="relative z-10 w-full max-w-lg animate-in">
      <div
        class="glass-panel p-8 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.4)] border-white/10 space-y-8"
      >
        ...
        <form @submit.prevent="handleRegister" class="space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
              <label
                class="text-[9px] font-black text-pink-300 uppercase ml-2 tracking-widest"
                >Name</label
              >
              <input
                v-model="nickname"
                ...
                class="input-glass"
                :class="{
                  'border-rose-500/50': nickname.length > 0 && !isNicknameValid,
                }"
              />
            </div>
            ...
            <div class="space-y-1">
              ...
              <input
                ...
                maxlength="10"
                :class="{
                  'border-rose-500/50': phoneNumber.length > 0 && !isPhoneValid,
                }"
              />
            </div>
          </div>
          ...
          <div class="space-y-1">
            ...
            <input
              v-model="email"
              type="email"
              placeholder="name@transmission.com"
              class="input-glass"
              :class="{
                'border-rose-500/50': email.length > 0 && !isEmailValid,
              }"
            />
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div
              class="bg-black/20 p-5 rounded-[1.5rem] border border-white/5 space-y-3"
            >
              <p
                class="text-[8px] font-black text-white/40 uppercase tracking-widest border-b border-white/5 pb-2"
              >
                Complexity Requirements
              </p>
              <ul class="space-y-2">
                <li
                  v-for="check in constraintChecks"
                  :key="check.text"
          ...
          >
            <div
              v-if="loading"
              class="absolute inset-0 bg-indigo-900 flex items-center justify-center"
            >
              <div
        ...
        <div class="text-center pt-4">
          ...
          >
            ...
            <NuxtLink
              to="/login"
              class="text-pink-400 hover:text-white transition underline decoration-pink-500/30 underline-offset-4"
              >Log In</NuxtLink
            >
          </p>
        </div>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div
        v-if="showModal"
        class="fixed inset-0 bg-indigo-950/90 backdrop-blur-xl z-[100] flex items-center justify-center p-4"
      >
        <div
          class="glass-panel p-10 max-w-sm text-center border-white/20 animate-in"
        >
          <div
            class="w-20 h-20 bg-emerald-500/10 rounded-[2rem] flex items-center justify-center mx-auto mb-6 border border-emerald-500/30"
          >
          ...
          <p
            class="text-white/60 text-xs mt-3 mb-8 leading-relaxed uppercase tracking-widest font-bold"
          >
            Your digital fingerprint has been added to the fluid stream.
          </p>
          <button
            @click="closeModalAndRedirect"
            class="w-full py-4 bg-white text-indigo-950 font-black rounded-2xl hover:bg-pink-300 transition-all shadow-xl uppercase text-xs tracking-widest"
          >
            Enter Transmission
          </button>
        </div>
