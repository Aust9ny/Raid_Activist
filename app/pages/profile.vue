...
// --- Background Setup ---
const { dotPositions, ripples, handleBackgroundClick } =
...
const getFullAvatarUrl = (path) => {
  if (!path)
    return "https://ui-avatars.com/api/?name=" + userData.value.username;
...
<template>
  <div
    class="profile-root min-h-screen relative flex flex-col p-4 md:p-8 overflow-x-hidden"
  >
    ...
    <div class="w-full max-w-5xl mx-auto space-y-6 relative z-10">
      <header
        class="glass-panel flex items-center justify-between p-5 px-8 shadow-2xl border-white/20"
      >
        <div class="flex items-center gap-4">
          <div
            class="p-2.5 bg-gradient-to-br from-pink-500 to-indigo-600 rounded-2xl shadow-lg"
          >
        ...
        <NuxtLink
          to="/dashboard"
          class="group flex items-center gap-2 text-white/60 hover:text-white transition-all font-bold uppercase text-xs tracking-widest"
        >
      ...
      <nav
        class="flex p-1.5 bg-black/20 backdrop-blur-md rounded-[2rem] border border-white/5 shadow-inner"
      >
        <button
          ...
          :class="[
            ...
            activeTab === 'profile'
              ? 'bg-white text-indigo-900 shadow-xl scale-[0.98]'
      ...
      <main>
        <div
          class="glass-panel p-8 md:p-16 shadow-2xl relative overflow-hidden"
        >
          <div
            class="absolute -top-24 -right-24 w-64 h-64 bg-pink-500/10 rounded-full blur-[100px]"
          ></div>

          <div v-if="isPageLoading" class="flex flex-col items-center py-20">
            <div
              class="w-16 h-16 border-4 border-white/5 border-t-pink-500 rounded-full animate-spin mb-6"
            ></div>
            <p
              class="text-[10px] font-black uppercase tracking-[0.4em] text-white/40"
            >
          ...
          <div
            ...
            class="animate-in space-y-12"
          >
            <div class="flex flex-col md:flex-row items-center gap-10">
              <div class="relative group">
                <div
                  class="absolute inset-0 bg-pink-500 rounded-full blur-2xl opacity-0 group-hover:opacity-20 transition-opacity"
                ></div>
                <img
                  :src="
                    avatarPreview || getFullAvatarUrl(userData.profileImageUrl)
                  "
                ...
                />
                <label
                  class="absolute -bottom-2 -right-2 bg-white p-3 rounded-2xl cursor-pointer shadow-xl hover:scale-110 active:scale-90 transition-all"
                >
                  <svg
              ...
              <div class="text-center md:text-left">
                <h2
                  class="text-6xl font-black text-white tracking-tighter mb-2"
                >
                  {{ userData.name }}
                </h2>
                <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div
                class="md:col-span-2 glass-panel p-8 bg-white/5 border-white/10 group"
              >
                <h3
                  class="text-[10px] font-black uppercase tracking-widest text-pink-400 mb-4 flex items-center gap-2"
                >
                  <span class="w-2 h-2 bg-pink-500 rounded-full"></span> Core
                  Bio
                </h3>
                <p
                  class="text-xl text-white/80 leading-relaxed font-medium italic"
                >
                  "{{ userData.bio }}"
                </p>
              </div>
              ...
              <div class="space-y-4">
                <div class="glass-panel p-5 bg-white/5 border-white/10">
                  <p
                    class="text-[9px] font-black text-white/30 uppercase tracking-widest mb-2"
                  >
                    Location Node
                  </p>
                ...
                <div class="glass-panel p-5 bg-white/5 border-white/10 group">
                  <div class="flex justify-between items-start mb-2">
                    ...
                    <button
                      @click="
                        isSensitiveVisibleEmail = !isSensitiveVisibleEmail
                      "
                      class="text-[9px] font-black text-pink-500 uppercase tracking-tighter hover:underline"
                    >
                ...
                <div class="glass-panel p-5 bg-white/5 border-white/10">
                  <div class="flex justify-between items-start mb-2">
                    ...
                    <button
                      @click="isSensitiveVisibleTel = !isSensitiveVisibleTel"
                      class="text-[9px] font-black text-pink-500 uppercase tracking-tighter hover:underline"
                    >
            ...
            <div class="pt-8 flex flex-col items-center">
              <button
                @click="handleLogout"
                class="group px-12 py-4 rounded-2xl bg-rose-500/10 text-rose-500 font-black text-[10px] tracking-[0.3em] uppercase border border-rose-500/20 hover:bg-rose-500 hover:text-white transition-all duration-500"
              >
                Disconnect Account
              </button>
            </div>
          ...
          <div v-else class="animate-in space-y-16">
            <section class="space-y-8">
              <div class="flex items-center gap-4">
                ...
                <h3
                  class="text-[10px] font-black text-white/40 uppercase tracking-[0.4em]"
                >
                  Personal Data
                </h3>
                <div class="h-px flex-1 bg-white/10"></div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                  <label
                    class="text-[10px] font-black text-white/30 uppercase tracking-widest ml-1"
              ...
              <div class="space-y-3">
                ...
                <textarea
                  ...
                  class="input-glass min-h-[120px] resize-none"
              ...
              <div class="flex justify-end">
                <button
                  ...
                  class="px-10 py-4 bg-white text-indigo-900 font-black text-[10px] tracking-widest uppercase rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-2xl"
                >
                  Push Changes
                </button>
              </div>
            ...
            <section class="space-y-8">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div
                  class="glass-panel p-6 bg-white/5 border-white/5 space-y-4"
                >
                  <p
                  ...
                  <div class="space-y-3">
                    <div
                      v-for="constraint in constraintStatus"
                      :key="constraint.text"
                      class="flex items-center gap-3 transition-all duration-300"
                      :class="constraint.isMet ? 'opacity-100' : 'opacity-40'"
                    >
                      <div
                        class="w-4 h-4 rounded-md flex items-center justify-center border transition-colors"
                        :class="
                          constraint.isMet
